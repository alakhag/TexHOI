...
class IDRTrainRunner():
    def __init__(self,**kwargs):
        ...
        implicit_model_test = torch.load("/home/alakhaggarwal/IRHOI/IRHOI_stage-1/code/logs/{:s}/checkpoints/object_implicit.pth".format(kwargs['expname']))
        self.model.implicit_network.load_state_dict(implicit_model_test)
    ...
    def plot_to_disk(self):
        ...
        # import trimesh
        # mesh = trimesh.load('/home/alakhaggarwal/Downloads/DexYCB/models/021_bleach_cleanser/textured_simple.obj')
        # # mesh.vertices = mesh.vertices * 0.125
        # sample_pts = mesh.sample(1000)
        ...
        # for i in [3]:
        # for i in range(1):
        # for i in range(0, len(self.train_dataset), 20):
        ...
        # pixel_camera = np.matmul(R, sample_pts.T).T + t.reshape((1,3))
        # pixel_camera[:,1] *= -1
        # pixel_camera[:,2] *= -1
        # xyz = np.matmul(K, pixel_camera.T).T
        # xy1 = xyz / xyz[:,2:]
        # ij = xy1[:,:2]
        # ij[ij<0] = 0
        # ij[ij[:,0]>639,0] = 639
        ...
        # rgb = ground_truth['rgb'].detach().cpu().numpy()
        # rgb = rgb[0].reshape((480,640,3))
        # rgb[ij[:,1].astype(np.int), ij[:,0].astype(np.int), :] = 0
        # imageio.imwrite(os.path.join("test", 'rgb_{}.png'.format(i)), rgb)
        # continue
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.nepochs + 1):
            ...
            for data_index, (indices, model_input, ground_truth) in enumerate(self.train_dataloader):
                ...
                if (self.cur_iter % self.ckpt_freq == 0) and self.cur_iter!=0:
                    self.save_checkpoints(epoch)
                ...
                # if self.loss.r_patch < 1:
                #     self.train_dataset.change_sampling_idx(self.num_pixels, indices)
                # else:
                #     self.train_dataset.change_sampling_idx(self.num_pixels,
                #         # self.num_pixels // (4*self.loss.r_patch*self.loss.r_patch),
                #                                                 # self.loss.r_patch,
