...
from training.idr_render import IDRTrainRunner
...
if __name__ == '__main__':
    ...
    parser.add_argument('--write_idr', default=False, action="store_true",
                        help='')
    parser.add_argument('--skipiter', type=int, default=1, help='skipping iterations of dataset to render')

    opt = parser.parse_args()

    if opt.gpu == "auto":
        deviceIDs = GPUtil.getAvailable(order='memory', limit=2, maxLoad=0.5, maxMemory=0.5, includeNan=False, excludeID=[], excludeUUID=[])
        gpu = deviceIDs[0]
    else:
        gpu = opt.gpu
