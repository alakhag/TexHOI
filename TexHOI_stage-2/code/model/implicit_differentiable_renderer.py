...
class ImplicitNetwork(nn.Module):
    ...
    def forward(self, input, compute_grad=False):
        ...
        for l in range(0, self.num_layers - 1):
            ...
            if l == 0:
                x = torch.cat([x, torch.zeros(x.shape[0], 0).cuda()], dim=-1)
...
class IDRNetwork(nn.Module):
    def __init__(self, conf, DATA_LEN=0):
        super().__init__()
        ...
        # pose_corr_v = torch.zeros((DATA_LEN,7))
        # pose_corr_v[:,0] = 1.0
        # self.pose_corr_vecs = nn.Parameter(pose_corr_v, requires_grad=True)
    ...
    def forward(self, input):
        ...
        ho_mask = input["ho_mask"].reshape(-1)
        w2c = input["pose_inv"]
        hand_params = input["hand_params"]
        # w2c = None
        # hand_params = None
        ...
        # ray_dirs, cam_loc = rend_util.get_camera_params_rotational(uv, idx=i)

        uv = uv.detach().cpu().numpy()
        # indices = np.random.choice(uv.shape[1], 32, replace=False)
        # uv = uv[:,indices,:]
        pose = pose.detach().cpu().numpy()
        intrinsics = intrinsics.detach().cpu().numpy()
        ray_dirs, cam_loc = rend_util.get_camera_params_ho3d_b4(uv, pose, intrinsics)

        # pose_corr = self.pose_corr_vecs[input["idx"]]
        # r_corr = rend_util.quat_to_rot(pose_corr[:,:4])
        # t_corr = pose_corr[:,4:]

        # # ray_dirs = torch.bmm(r_corr, ray_dirs.transpose(2,1)).transpose(2,1)
        # # cam_loc = torch.bmm(r_corr, cam_loc.unsqueeze(-1)).transpose(2,1).squeeze(1)
        
        # import trimesh
        # mesh = trimesh.load("/home/alakhaggarwal/Downloads/DexYCB/models/004_sugar_box/textured_simple.obj")
        # cam_loc_ = cam_loc[0].detach().cpu().numpy()
        # ray_dirs_ = ray_dirs[0].detach().cpu().numpy()
        # ray_dirs_ = ray_dirs_ / np.linalg.norm(ray_dirs_, axis=1, keepdims=True)
        # ray_mesh = []
        # for i, ray_dir in enumerate(ray_dirs_):
        #     ray = trimesh.creation.cylinder(radius=0.002, segment=[cam_loc_, cam_loc_+ray_dir])
        #     ray_mesh.append(ray)

        # sph = trimesh.creation.uv_sphere(radius=0.002)
        # sph = sph.apply_translation(cam_loc_)
        # ray_mesh.append(sph)
        # ray_mesh = trimesh.util.concatenate(ray_mesh)

        batch_size, num_pixels, _ = ray_dirs.shape

        object_mask, ho_mask = ho_mask, object_mask
        ...
        # pc = trimesh.PointCloud(points.detach().cpu().numpy().reshape((-1,3)))
        # scene = trimesh.scene.Scene()
        # scene.add_geometry(mesh)
        # scene.add_geometry(ray_mesh)
        ...
        sg_diffuse_rgb_values = torch.zeros_like(points).float().cuda()
        sg_diffuse_albedo_values = torch.zeros_like(points).float().cuda()
        specular_rgb_diff_values = torch.zeros_like(points).float().cuda()
        sg_specular_rgb_values = torch.zeros_like(points).float().cuda()
        sg_specular_original = torch.zeros_like(points).float().cuda()
        sg_diffuse_original = torch.zeros_like(points).float().cuda()
        if differentiable_surface_points.shape[0] > 0:
            view_dirs = -ray_dirs[surface_mask]  # ----> camera
            ret = self.get_rbg_value(differentiable_surface_points, view_dirs, hand_params=hand_params, w2c=w2c)
            ...
            specular_rgb_diff_values[surface_mask] = ret['specular_rgb_diff']

            sg_specular_original[surface_mask] = ret['sg_specular_original']
            sg_diffuse_original[surface_mask] = ret['sg_diffuse_original']

        object_mask, ho_mask = ho_mask, object_mask

        # network_object_mask = (network_object_mask & ho_mask)
        # network_object_mask = network_object_mask.logical_or(ho_mask) # include hand mask in object mask prediction
        ...
        output = {
            ...
            'specular_rgb_diff_values': specular_rgb_diff_values,
            ...
            'sg_specular_original': sg_specular_original,
            'sg_diffuse_original': sg_diffuse_original,
        }

        return output

    def get_rbg_value(self, points, view_dirs, hand_params=None, w2c=None):
        ...
        ### sg renderer
        sg_envmap_material = self.envmap_material_network(points, w2c=w2c)
        sg_ret = render_with_sg(lgtSGs=sg_envmap_material['sg_lgtSGs'],
                                ...
                                point=points, normal=normals, viewdirs=view_dirs,
                                blending_weights=sg_envmap_material['sg_blending_weights'],
                                skin_color=sg_envmap_material['skin_color'],
