...
class RayTracing(nn.Module):
    ...
    def sphere_tracing(self, batch_size, num_pixels, sdf, cam_loc, ray_directions, mask_intersect, sphere_intersections):
        ...
        # pc0 = trimesh.PointCloud(sphere_intersections_points[0,:,0].detach().cpu().numpy())
        # pc1 = trimesh.PointCloud(sphere_intersections_points[0,:,1].detach().cpu().numpy())
        # sph = trimesh.creation.uv_sphere(radius=0.02)
        # sph = sph.apply_translation(cam_loc.reshape(3).detach().cpu().numpy())
        # # sph_1 = trimesh.creation.uv_sphere(radius=1.0)
        # # pc_sph1 = trimesh.PointCloud(sph_1.vertices)
        # scene = trimesh.scene.Scene()
        # scene.add_geometry(pc0)
        # scene.add_geometry(pc1)
        # scene.add_geometry(mesh)
        # # scene.add_geometry(pc_sph1)
    ...
    def ray_sampler(self, sdf, cam_loc, object_mask, ray_directions, sampler_min_max, sampler_mask):
        ...
        if n_rootfind_pts > 0:
            ...
            z_pred_rootfind = self.rootfind(sdf_low, sdf_high, z_low, z_high, cam_loc_rootfind, ray_directions_rootfind, sdf)
    ...
    def rootfind(self, sdf_low, sdf_high, z_low, z_high, cam_loc, ray_directions, sdf):
