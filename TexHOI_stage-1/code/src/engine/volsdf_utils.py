...
def compute_gradient_samples(
    ...
    local_pred = implicit_network(sample, cond, None)[..., 0:1]
...
def extract_features(
    ...
    output = implicit_network(pnts_c, cond, node_id="not object")
...
def render_fg_rgb(
    ...
    #         hand_betas = right_node.params.betas.weight.expand((num_images, -1))
    #         global_orient = right_node.params.global_orient.weight[input_idx]
    #         pose = right_node.params.pose.weight[input_idx]
    #         hand_poses = torch.cat((global_orient, pose), dim=-1)
    #         hand_trans = right_node.params.transl.weight[input_idx]
    #         hand_params = {
    #             'beta': hand_betas,
    #             'theta': hand_poses,
    #             'trans': hand_trans,
    #             'tfm': torch.inverse(tfs[:,0])
    #         }
    #     fg_rendering_output = rendering_network(
    #         pnts_c, normals, view_dirs, cond["pose"], feature_vectors, deformer, tfs, right_node=None, left_node=None, stage=stage, hand_params=hand_params
    #         )
    #     return fg_rendering_output, normals
    # else:
    fg_rendering_output = rendering_network(
...
def sdf_func_with_deformer(deformer, sdf_fn, training, x, deform_info, node_id):
    ...
    output = sdf_fn(x_c, cond, node_id)
    sdf = output[:, :, 0:1]
    feature = output[:, :, 1:]
    # if not training:
...
def density2weight(density_flat, z_vals, z_max):
    ...
    fg_weights = alpha * fg_transmittance  # probability of the ray hits something here # (num_rays, 1)
    if z_vals.shape[1] == 1:
        fg_weights = density
        bg_weights = (1-density)[:,0]
