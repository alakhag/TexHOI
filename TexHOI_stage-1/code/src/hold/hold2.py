...
from src.hold.loss import Loss_stage2
from src.hold.hold_net2 import HOLDNet
...
class HOLD_stage2(pl.LightningModule):
    def __init__(self, opt, args) -> None:
        ...
        self.model.nodes["object"].set_stage2()

        for node in self.model.nodes.values():
            node.params.freeze()
            for params in node.implicit_network.parameters():
                params.requires_grad = False
            
            if node.node_id != "object":
                for params in node.rendering_network.parameters():
                    params.requires_grad = False

        for params in self.model.background.parameters():
            params.requires_grad = False

        self.loss = Loss_stage2(args)
    ...
    def configure_optimizers(self):
        ...
        params_lgtsg = self.model.nodes["object"].rendering_network.envmap_material_network.lgtSGs
        param_skin = self.model.nodes["object"].rendering_network.skin_color
        params = list(self.model.nodes["object"].rendering_network.envmap_material_network.brdf_layer.parameters())
        params.append(params_lgtsg)
        params.append(param_skin)
    ...
    def training_step(self, batch):
        ...
        # split = split_input(batch, batch["uv"].shape[1], 16)
        # out_list = []
        # for s in split:
        #     out = self.model(s)
        #     out_list.append(out)
        
        # model_outputs = merge_output(out_list, batch["uv"].shape[1], batch_size)
    ...
    def inference_step(self, batch, *args, **kwargs):
        ...
        if not self.args.no_vis:
            ...
            for s in pbar:
                ...
                vis_dict.update(out.search("albedo"))
                vis_dict.update(out.search("specular"))
