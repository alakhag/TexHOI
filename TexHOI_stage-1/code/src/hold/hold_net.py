...
class HOLDNet(nn.Module):
    ...
    def forward_fg(self, input):
        ...
        for key in self.nodes.keys():
            node = self.nodes[key]
            if key=="object":
                # factors, sample_dict = node(input, self.nodes["right"])
        ...
        # v_o = sample_dicts["object"]["deform_info"]["verts"].reshape((-1,3)).detach().cpu().numpy()
        # c_o = np.zeros_like(v_o)
        # c_o[:,1] = 255
        # v_h = sample_dicts["right"]["deform_info"]["verts"].reshape((-1,3)).detach().cpu().numpy()
        # c_h = np.zeros_like(v_h)
        # c_h[:,0] = 255
        # pc_o = trimesh.PointCloud(v_o, c_o)
        # pc_h = trimesh.PointCloud(v_h, c_h)
        # scene = trimesh.scene.Scene()
        # scene.add_geometry(pc_o)
        # scene.add_geometry(pc_h)
        # scene.show()
        # exit()
    ...
    def forward(self, input):
        ...
        # bg_dict = {
        #     "bg_rgb": torch.zeros_like(fg_dict["fg_rgb"]),
        #     "bg_rgb_only": torch.zeros_like(fg_dict["fg_rgb"]),
        #     "bg_semantics": torch.zeros_like(fg_dict["fg_semantics"])
        # }
