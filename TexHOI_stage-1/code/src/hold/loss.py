...
class Loss_stage2(nn.Module):
    ...
    def forward(self, batch, model_outputs):
        ...
        # print (model_outputs["rgb"].shape)
        # print (model_outputs["rgb"][obj_mask].shape)
        # print (rgb_gt.shape)
        # print (rgb_gt[obj_mask].shape)
        # exit()

        rgb_loss = nn.L1Loss()(
            model_outputs["rgb"][obj_mask],
            rgb_gt[obj_mask]
        )

        loss_dict = {"loss": rgb_loss}
        return loss_dict
...
class Loss(nn.Module):
    ...
    def forward(self, batch, model_outputs):
        ...
        #         print (k, ':{:.3f}'.format(loss_dict[k].item()), end=',')
        #     else:
        #         print (k, ':{:.3f}'.format(loss_dict[k]), end=',')
        # print ()
