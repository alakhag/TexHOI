...
def query_oc(implicit_fn, x, cond):
    ...
    mnfld_pred = implicit_fn(x, cond, None)
...
def merge_factors(factors_list, check=True):
    ...
    for i in range(len(factors_list)-1):
        factors_list[i]["node_color"] = torch.zeros_like(factors_list[i]["color"])
    
    factors_list[-1]["node_color"] = torch.ones_like(factors_list[-1]["color"])
    ...
    # factors_comp["node_color"] = torch.cat([factors["node_color"] for factors in factors_list], dim=1)

    # sort z
    z_vals = factors_comp["z_vals"]
    z_vals_sorted, indices = torch.sort(z_vals, dim=1)
    z_vals = sort_tensor(z_vals[:, :, None], indices)[:, :, 0]
    assert torch.abs(z_vals - z_vals_sorted).max() < 1e-6, "z_vals not sorted, {:f}, {:f}".format(torch.abs(z_vals - z_vals_sorted).max(), torch.abs(z_vals - z_vals_sorted).min())
...
def volumetric_render(factors, is_training):
    ...
    if "node_color" in factors.keys():
        fg_ho_mask = integrate(factors["node_color"][:, :, :1], fg_weights)
        out_dict["ho_mask"] = torch.clamp(fg_ho_mask, 0, 1) 

    if "albedo" in factors.keys():
        albedo = factors["albedo"]
        specular = factors["specular"]
        fg_albedo = integrate(albedo, fg_weights)
        fg_specular = integrate(specular, fg_weights)
        out_dict["fg_albedo"] = fg_albedo
        out_dict["fg_specular"] = fg_specular

    if not is_training:
        bg_rgb = torch.ones_like(fg_rgb, device=fg_rgb.device)
        out_dict["fg_rgb.vis"] = fg_rgb + bg_weights[:, None] * bg_rgb
        if "albedo" in factors.keys():
            out_dict["fg_albedo.vis"] = fg_albedo + bg_weights[:, None] * bg_rgb
            out_dict["fg_specular.vis"] = fg_specular + bg_weights[:, None] * bg_rgb
    return out_dict
