...
from src.model.renderables.node2 import Node
...
class ObjectNode(Node):
    ...
    def sample_points(self, input):
        ...
        # uv = np.mgrid[: 480, : 640].astype(np.int32)
        # uv = np.flip(uv, axis=0).copy().transpose(1, 2, 0).astype(np.float32)
        # uv = uv.reshape(1, -1, 2).astype(np.float32)
        # idxs = np.random.choice(uv.shape[1], 512, replace=True)
        # uv = torch.from_numpy(uv[:,idxs,:]).cuda()
        ray_dirs, cam_loc = get_camera_params(
            input["uv"], input["extrinsics"], input["intrinsics"]
            # uv, input["extrinsics"], input["intrinsics"]
        )
        ...
        with torch.no_grad():
            ...
            for i in range(n_images):
                ...
                cond = {"pose": obj_cond["pose"][i].unsqueeze(0)}

                cur_pts, network_object_mask, cur_z = self.ray_tracer(sdf=lambda x: hold_utils.query_oc(self.implicit_network, x, cond)['sdf'],
                # cur_pts, network_object_mask, cur_z = self.ray_tracer(sdf=lambda x: self.implicit_network(x, cond, "object")[:,:,0],
        ...
        net_masks = torch.cat(net_masks, dim=0).reshape(-1,1)
